import { isComplexPluginOutput } from '@graphql-codegen/plugin-helpers';
...
export async function codegen(options) {
    ...
    if (schemaChanged) {
        ...
        options.schemaAst = buildASTSchema(schema, options.config.federation
            ? {
                assumeValidSDL: true,
            }
            : undefined);
...
function validateDocuments(files) {
  ...
  `.trimRight())
            .join('');
        throw new DetailedError(`Not all operations have an unique name: ${duplicated.join(', ')}`, `
